// CAPS-Language Frontend Main Entry Point
// This file orchestrates the compilation pipeline for CAPS-Language,
// a language focused on deterministic processes, typed channels with ownership transfer,
// no shared memory, and strong compile-time guarantees.
// Now massively upgraded with fixed-size buffers, bounded queues, deterministic timers,
// deterministic I/O, process orchestration tools, static math utilities, and safe FFI to C.

// Key Concepts Enforced:
// - Deterministic Processes: FSMs with total transitions, explicit scheduling.
// - Typed Channels: Bounded, type-safe communication with ownership transfer.
// - Ownership Transfer: Send operations move data, preventing races.
// - No Shared Memory: Private process state; channels as sole communication.
// - Compile-Time Guarantees: Type safety, bounds, topology validated statically.
// - New Upgrades: Fixed-size buffers (compile-time sized), bounded queues (channels),
//   deterministic timers (monotonic), deterministic I/O (bounded ops),
//   orchestration (spawn/join/monitor), static math (compile-time funcs), safe FFI (type-checked calls).
...
struct Options {
  ...
  std::string target_arch = "x86_64";  // Default target architecture
};


static void print_usage() {
  std::cerr <<
    "usage: caps_frontend [--dump-ast] [--dump-topology=dot|text] [--check-only] [--output-ir=<file>] [--emit-cpp=<dir>] [--compile] [--emit-asm=<file>] [--emit-obj=<file>] [--target-arch=<arch>] <file.caps>\n"
    "\n"
    ...
    "  --target-arch=<arch>   Set target architecture (x86_64, arm64, riscv, wasm)\n";
}
...
static bool parse_args(int argc, char** argv, Options& opt) {
  for (int i = 1; i < argc; i++) {
    ...
    if (a.rfind("--target-arch=", 0) == 0) {
      auto eq = a.find('=');
      if (eq == std::string::npos) {
        std::cerr << "error: --target-arch requires '=arch'\n";
        return false;
      }
      opt.target_arch = a.substr(eq + 1);
      continue;
    }
...
int main(int argc, char** argv) {
  ...
  Sema sema(diag);
  sema.check(prog);
  sema.infer_types(prog);
  sema.check_lifetimes(prog);
  sema.borrow_check(prog);
  sema.determinism_check(prog);
  sema.check_advanced(prog);  // Existing advanced checks
  ...
  if (!opt.check_only) {
    ...
    for (auto& g : prog.groups) {
      ...
      // Backend selection based on target
      std::string target = opt.target_arch;
      if (target == "x86_64") {
        // Emit x64
        ...
        std::cerr << "error: unknown target architecture: " << target << "\n";
        return 1;
      }
    }
  }
